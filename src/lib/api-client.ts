...
export interface AlertaVenta {
  ...
  estado: EstadoAlerta
  creadaEn: string
  prospecto: { id: string; nombre: string; email: string; cargo: string }
  vendedor: { id: string; nombre: string; region: string } | null
}
...
export async function getAlertas(vendedorId?: string): Promise<AlertaVenta[]> {
  ...
  const res = await fetch(`${BASE_URL}/alertas-ventas${params}`, { cache: 'no-store' })
  if (!res.ok) throw new Error('Error al obtener alertas')
  return res.json()
}

export async function updateEstadoAlerta(id: string, estado: EstadoAlerta): Promise<AlertaVenta> {
...
export async function verificarVencimientos(region?: string): Promise<VerificacionResult> {
